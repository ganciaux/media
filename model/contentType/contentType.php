...
class contentType{
	...
	static function exists($name,$id){
		...
		$req = $bdd->prepare("select count(*) from content_type where search=:search and idContentType<>:id");
	...
	function insert(){
		...
		$req = $bdd->prepare('insert into content_type (name,search) values (:name,:search)');
	...
	function update(){
		global $bdd;
		$search=setSearchString($this->name);
		$req = $bdd->prepare('update content_type set name=:name, search=:search where idContentType=:id');
